...
int			is_separator(char c, char *charset) //  charset = "test" 
{
	int i;

	i = -1;
	if (!charset)// charset existe !!
		return (1);
	while (charset[++i])//je boucle sur charset dans "test"
	{
		// 1er boucle chaset[i] = t
		if (charset[i] == c) //si c = t
...
int			count_nb_words(char *str, char *charset)
{
	...
	while (str[++i])
	{
		if (is_separator(str[i], charset) // je suis sur un separateur 
				&& str[i - 1] && 
...
char		**ft_split(char *str, char *charset) // FT_SPLIT
{
	char	**splitey; // ~ tab
	int		nb_words;
